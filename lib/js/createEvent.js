  ...
  $('#createEventSend').on('click', function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        console.log(data);
        $('#mailCreateEvent').html("<div class='alert alert-success'>");
        $('#mailCreateEvent > .alert-success').html("<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;").append("</button>");
        $('#mailCreateEvent > .alert-success').append("<strong>Votre évènement a été crée. Vous allez recevoir un mail de confirmation afin de valider votre évènement ! </strong>");
        $('#mailCreateEvent > .alert-success').append('</div>');
        $('#createEventForm').trigger("reset");
        setTimeout(function() {
          $('#mailCreateEvent').html("");
        }, 7000);
      },
      error: function(data) {
        console.log(data);
        $('#mailCreateEvent').html("<div class='alert alert-danger'>");
        $('#mailCreateEvent > .alert-danger').html("<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;").append("</button>");
        $('#mailCreateEvent > .alert-danger').append($("<strong>").text("Désolé, il semble que le serveur ne réponde pas ! Veuillez réésayer plus tard, merci ! "));
        $('#mailCreateEvent > .alert-danger').append('</div>');
        $('#createEventForm').trigger("reset");
        setTimeout(function() {
          $('#mailCreateEvent').html("");
        }, 3000);
