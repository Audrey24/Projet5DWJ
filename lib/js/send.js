...
var imagesPreview = function(input, place, trueName) {
            ...
            if(type=='image') {
              ...
              var name = input.name.split('.')[0] + '.mp4';

              var container = $('<div class="containImg" data-id="'+ name +'"></div>');
              $( "<p class='closeImg' data-id='"+ name +"'>&times;</p>" ).appendTo(container);
              var videoContainer = $('<video class="preview" width="400" controls autoplay><source src='+video+'></video>');
  ...
  worker.onmessage = function (event) {
    ...
    if (message.type == "ready") {
      ...
      message.data.forEach(function(file) {
        fileList.append(file.name, file.data);
      });
    }
  };
    ...
    $('#my-file').on('change', function() {
        for (var i = 0; i < this.files.length; i++) {
          if(!fileList.has(this.files[i].name)) {
            var trueName = num.toString();
            num++;
            imagesPreview(this.files[i], 'div.gallery', trueName);
            // on creer la key utilisee dans send model pour la boucle foreach
            var name = this.files[i].name.split(".");
            var len = name.length-1;

            var reader2 = new FileReader();
            var res = new Uint8Array(reader2.readAsArrayBuffer(this.files[i]));
            var results = worker.postMessage({
              type: 'command',
              arguments: '-i bidon.'+ name[len] +' -vf showinfo -strict -2 -c:v libx264 '+ trueName +'.mp4',
              files: [
                {
                  data: res,
                  name: "bidon." + name[len]
