    ...
    var imagesPreview = function(input, place) {
        ...
        if (input.files) {
            ...
            for (i = 0; i < filesAmount; i++) {
                var type = input.files[i].type.split('/')[0];
                ...
                if(type=='image') {
                  ...
                  var video = URL.createObjectURL(input.files[i]);
                  var container = $('<div class="containImg"></div>');

                  $( "<p class='closeImg'>&times;</p>" ).appendTo(container);
                  var videoContainer = $('<video width="400" controls><source src='+video+'></video>');
                  ...
                  //TODO: message erreur type non accepté
                }
            }
        }
    };
